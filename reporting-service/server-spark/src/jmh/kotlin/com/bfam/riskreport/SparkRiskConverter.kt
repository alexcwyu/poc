...
import com.memsql.spark.connector.util.*
import com.memsql.spark.connector.util.JDBCImplicits.*
import java.sql.Connection
import java.sql.DriverManager
import java.sql.ResultSet

open class SparkMemSQLRiskRunner {
    ...
    open class Session {
        ...
        lateinit var connInfo: MemSQLConnectionInfo
        lateinit var conn: Connection

        @Setup(Level.Invocation)
        fun init() {

            connInfo = MemSQLConnectionInfo("127.0.0.1", 3306, "root", "", "analytic")

            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/analytic", "root", "")
            ...
            sparkConf.set("spark.memsql.host", connInfo.dbHost())
            sparkConf.set("spark.memsql.port", connInfo.dbPort().toString())
            sparkConf.set("spark.memsql.user", connInfo.user())
            sparkConf.set("spark.memsql.password", connInfo.password())
            sparkConf.set("spark.memsql.defaultDatabase", connInfo.dbName())
            sparkConf.setJars(arrayOf("/Users/alex/dev/workspaces/poc/reporting-service/server-spark/libs/memsql-connector_2.11-2.0.6.jar"
                    ,"/Users/alex/dev/workspaces/poc/reporting-service/server-spark/libs/commons-dbcp2-2.1.1.jar"
            ,"/Users/alex/dev/workspaces/poc/reporting-service/server-spark/libs/commons-pool2-2.4.2.jar",
                    "/Users/alex/dev/workspaces/poc/reporting-service/server-spark/libs/mysql-connector-java-5.1.47.jar"))
...
//            connectionProperties["Driver"] =  "com.mysql.jdbc.Driver"
//
//            bigJoin.write().jdbc("jdbc:mysql://localhost:3306/analytic", "bigjoin", connectionProperties)

            bigJoin.write().format("com.memsql.spark.connector").save("bigjoin")
    ...
    @Warmup(iterations = 5)
    fun groupby_memsql(session: Session) {
        var count = 0
        val queryPerf = measureTimeMillis {
            val rs: ResultSet = session.conn!!.createStatement().executeQuery("""
                SELECT `portfolio_name`, `inst_name`,
        ...
        println("-------memsql: $queryPerf $count")
    }
    ...
    fun groupby_2level(session: Session) {
        ...
        println("-------spark: $queryPerf $covPerf $encodePerf $decodePerf")
    }
...
fun main(args: Array<String>) {
    ...
    val runner = SparkMemSQLRiskRunner()
    val session = SparkMemSQLRiskRunner.Session()
    session.init()
